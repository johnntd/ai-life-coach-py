...
def build_session_prompt(payload: dict) -> str:
    ...
    # Enforce first-name addressing explicitly
    first = _first_name(payload.get("name") or "Friend")
    bits.append(f"GreetingName: {first}")
    bits.append("Always greet and address the user by first name only, not the full name.")
...
def _first_name(name: str) -> str:
    """Return first token of a name, handling 'Last, First' too."""
    s = (name or "").strip()
    if not s:
        return "Friend"
    if "," in s:
        ...
        if len(parts) >= 2:
            s = parts[1]  # 'Last, First' -> First
        else:
            s = parts[0]
    return s.split()[0]

# --- Seed greeting helpers (en/vi) ---
def _vn_addr(age: int, sex: str) -> str:
...
def default_seed_line(name: str, age: int, mode: str, lang: str, sex: str = "unknown") -> str:
    """Two very short sentences, tutor-y for kids/teens, coach-y for adults. Uses first name only."""
    first = _first_name(name)
    lang = (lang or "en-US").lower()

    if lang == "vi-vn":
        you = _vn_addr(age or 0, sex)
        if (age or 0) < 13:
            # Tutor vibe
            return f"Chào {you} {first}! Mình cùng làm một trò học nhanh nhé? Em đang học gì để Sunny giúp."
        if (age or 0) < 18:
            # Tutor vibe
            return f"Chào {you} {first}! Mình kiểm tra nhanh rồi lập kế hoạch học ngắn nhé? Em muốn bắt đầu môn nào trước?"
        # Adult → coach vibe
        return f"Chào {you} {first}, mình là Sunny. Bạn muốn cải thiện điều gì nhất lúc này?"
    else:
        # en-US
        if (age or 0) < 13:
            return f"Hi, {first}! Want to try a quick learning game together? Tell me what you’re working on and I’ll help."
        if (age or 0) < 18:
            return f"Hey {first}, ready for a fast check-in and a mini study plan? Which class or topic first?"
        return f"Hi {first}, I’m Sunny. What’s one area you’d like to improve or feel stuck in right now?"
...
def compose_messages(payload: dict) -> list:
    ...
    if include_seed:
        # One or two short sentences, greet by first name, no age mention for adults.
        # Tutor stance if <18; coach stance if >=18. Reply in session language.
        name_full = (payload.get("name") or "Friend").strip()
        first     = _first_name(name_full)
        age       = int(payload.get("age") or 18)
        lang      = (payload.get("lang") or "en-US")
        you       = _vn_addr(age, payload.get("sex") or "unknown")

        if lang.lower() == "vi-vn":
            style = "gia sư" if age < 18 else "huấn luyện"
            seed = (
                f"Mở đầu thật ngắn bằng “Chào {you} {first}!”. "
                f"Giữ {style}, 1–2 câu ngắn, tự nhiên, dễ đọc TTS. "
                f"Hỏi đúng 1 câu ngắn để bắt đầu. Không nhắc đến tuổi."
            )
        else:
            stance = "tutor-y" if age < 18 else "coach-y"
            seed = (
                f"Start with a brief 'Hi, {first}!'. "
                f"Keep it {stance}, 1–2 short sentences, TTS-friendly. "
                f"Ask exactly one short question. Do not mention age."
            )
...
async def chat(request: Request):
    ...
    if not text:
        text = default_seed_line(name=name, age=age, mode=mode, lang=session_lang, sex=sex)
